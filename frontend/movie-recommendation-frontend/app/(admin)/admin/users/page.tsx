...
export default function UsersPage() {
  ...
  const { data: users = [], isLoading } = useQuery({
    ...
    queryFn: () => debouncedKeyword ? userService.searchUsers(debouncedKeyword) : userService.getAllUsers()
  });
    ...
    <div>
      ...
      <div className="rounded-md border border-white/10">
        <Table>
          ...
          <TableBody>
            {users.map((user: any) => (
              <TableRow key={user.publicId} className="border-white/10 hover:bg-white/5">
                <TableCell className="font-medium">{user.fullName}</TableCell>
                <TableCell className="text-zinc-400">{user.email}</TableCell>
                <TableCell>
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                    user.role === 'ADMIN' ? 'bg-red-500/10 text-red-500' : 'bg-blue-500/10 text-blue-400'
                  }`}>
                    {user.role}
                  </span>
                </TableCell>
                <TableCell>
                  <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${
                    user.status === 'ACTIVE' ? 'bg-green-500/10 text-green-500' : 'bg-red-500/10 text-red-400'
                  }`}>
                    {user.status}
                  </span>
                </TableCell>
                      ...
                      <>
                        <Button 
                          ...
                          onClick={() => {
                            ...
                            if (confirm(`Bạn có chắc muốn ${newStatus === 'ACTIVE' ? 'mở khóa' : 'khóa'} người dùng này?`)) {
                              statusMutation.mutate({ publicId: user.publicId, status: newStatus });
                            }
                          }} 
